...
CREATE PROCEDURE [dbo].[spDepartmentOpenWork]
...
AS 
	...
	where b.Name IN ('Open - Not Contacted', 'Working - Contacted') AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 

	Select a.Id, a.Name, a.CreatedDate as [Date], b.Name as [Status] from [Opportunity] a 
	inner join Stage b on a.StageId = b.Id
	where b.Name NOT IN ('Closed Won', 'Closed Lost') AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 
	...
	where b.Name IN ('Not Started', 'In Progress', 'Waiting on someone else') AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	...
	where b.Name IN ('New', 'Working', 'Escalated')  AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 
	...
	where b.Name IN ('In Review', 'Draft', 'Presented', 'Approved', 'Needs Review')  AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 
	...
	where b.Name IN ('Yet to begin' , 'In progress')  AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 
	...
	where b.Name NOT IN ('Closed')  AND 
	a.OrgId = @OrgId AND
	a.Active = 1 AND 
	a.OwnerId IN ( SELECT UserId From Hierarchy where DepartmentId = @DepartmentId )
	order by a.CreatedDate; 
