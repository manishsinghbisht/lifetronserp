var SAccount = new Object();

SAccount.init = function () {
    $(document).ready(SAccount.ready);
    $('#BillingAddressId').on('change', SAccount.AddressDdlChange);
    $('#ShippingAddressId').on('change', SAccount.AddressDdlChange);
    $("#btnSearch").click(SAccount.Search);
     $('input[type="submit"]').click(SAccount.Submit);
};

SAccount.Submit = function () {
    var btn = jQuery(this);
...
SAccount.Search = function () {
    //////$('#frmSearch').attr('action', 'Invoice/Search');
    //var pathname = window.location.pathname;
    //if (pathname.toLowerCase().indexOf("saccount/") >= 0) {

    //    $('form[name="frmSearch"]').attr('action', 'Search');
    //}
    //else {
    //    $('form[name="frmSearch"]').attr('action', 'SAccount/Search');
    //}
    //jQuery("#frmSearch").submit();
    $(this.form).submit();
};

SAccount.ready = function () {
    ...
    $("#stepWizardAccount").removeClass("btn-default");
    $("#stepWizardAccount").addClass("btn-primary");

    SAccount.ClearControls();
};

SAccount.ClearControls = function () {
    $("#Desc").val('');
};


SAccount.AddressDdlChange = function () {
    ...
    if (selectedVal != "") {
        SAccount.PostBackAddress(jQuery(this));
    }
};

SAccount.PostBackAddress = function (control) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (controlId.toLowerCase().indexOf("billing") >= 0) {
                SAccount.UpdateBillingAddressControls(response);
            } else {
                SAccount.UpdateShippingAddressControls(response);
            }
...
SAccount.UpdateBillingAddressControls = function (response) {
...
SAccount.UpdateShippingAddressControls = function (response) {
