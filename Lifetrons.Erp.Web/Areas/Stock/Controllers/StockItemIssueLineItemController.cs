...
namespace Lifetrons.Erp.Stock.Controllers
{
    ...
    public class StockItemIssueLineItemController : BaseController
    {
        private readonly IStockItemIssueService _service;
       ...
       // GET: /StockIssueLineItem/
        public StockItemIssueLineItemController(IStockItemIssueService service, IUnitOfWorkAsync unitOfWork, IStockIssueHeadService stockIssueHeadService,
        ...
        [EsAuthorize(Roles = "StockAuthorize, StockEdit")]
        public async Task<ActionResult> Create([Bind(Include = "StockIssueId,Serial,ItemId,JobNo,CaseNo,TaskNo,Quantity,Weight,WeightUnitId,ExtraQuantity,DeliveryDate,Remark,CustomColumn1,Authorized")] StockItemIssue instance)
        {
        ...
        public async Task<ActionResult> Edit(string id)
        {
            ...
            var applicationUser = new AccountController().GetUserById(User.Identity.GetUserId());
            StockItemIssue instance = await _service.FindAsync(id, applicationUser.Id, applicationUser.OrgId.ToString());
        ...
        private async Task SetupInstanceForm(ApplicationUser applicationUser, StockItemIssue instance)
        {
        ...
        public async Task<ActionResult> Edit([Bind(Include = "Id,StockIssueId,Serial,ItemId,JobNo,CaseNo,TaskNo,Quantity,Weight,WeightUnitId,ExtraQuantity,DeliveryDate,Remark,CustomColumn1,Authorized,Active,TimeStamp")] StockItemIssue instance)
        {
            if (User.Identity.IsAuthenticated)
            {
                var applicationUser = new AccountController().GetUserById(User.Identity.GetUserId());
                StockItemIssue model = await _service.FindAsync(instance.Id.ToString(), applicationUser.Id, applicationUser.OrgId.ToString());
                ...
                try
                {
                    if (TryValidateModel(model))
                    {
                        ...
                        var membersToCompare = new List<string>() { "Serial", "ItemId", "JobNo", "CaseNo", "TaskNo", "Quantity", "Weight", "WeightUnitId", "ExtraQuantity", "DeliveryDate", "Authorized", "Active" };
        ...
        public async Task<ActionResult> DeleteConfirmed(Guid id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                {
                    StockItemIssue model = await _service.FindAsync(id.ToString(), applicationUser.Id, applicationUser.OrgId.ToString());
                    parentGuid = model.StockIssueId.ToString();
                    string logEntry = "Deleted StockIssueDetail: Id=" + model.Id + " StockIssueId=" + model.StockIssueId + " ItemId=" + model.ItemId + "JobNo=" + model.JobNo + "CaseNo=" + model.CaseNo + "TaskNo=" + model.TaskNo;
        ...
        private void HandleDbUpdateException(DbUpdateException dbUpdateException)
        {
                ...
                innerExceptionMessage.Contains("PK"))
            {
                ...
                else if (innerExceptionMessage.Contains("UQ_StockIssueDetail_StockIssueId_ItemId_ProductId"))
                {
