...
namespace Lifetrons.Erp.Stock.Controllers
{
    ...
    public class StockProductReceiptLineItemController : BaseController
    {
        private readonly IStockProductReceiptService _service;
        ...
        public StockProductReceiptLineItemController(IStockProductReceiptService service, IUnitOfWorkAsync unitOfWork, IStockReceiptHeadService stockReceiptHeadService,
        ...
        [EsAuthorize(Roles = "StockAuthorize, StockEdit")]
        public async Task<ActionResult> Create([Bind(Include = "StockReceiptId,Serial,ProductId,JobNo,CaseNo,TaskNo,Quantity,Weight,WeightUnitId,ExtraQuantity,Expenses,Remark,CustomColumn1,Authorized")] StockProductReceipt instance)
        {
        ...
        public async Task<ActionResult> Edit(string id)
        {
            ...
            var applicationUser = new AccountController().GetUserById(User.Identity.GetUserId());
            StockProductReceipt instance = await _service.FindAsync(id, applicationUser.Id, applicationUser.OrgId.ToString());
        ...
        private async Task SetupInstanceForm(ApplicationUser applicationUser, StockProductReceipt instance)
        {
        ...
        public async Task<ActionResult> Edit([Bind(Include = "Id,StockReceiptId,Serial,ProductId,JobNo,CaseNo,TaskNo,Quantity,Weight,WeightUnitId,ExtraQuantity,Expenses,Remark,CustomColumn1,Authorized,Active,TimeStamp")] StockProductReceipt instance)
        {
            if (User.Identity.IsAuthenticated)
            {
                var applicationUser = new AccountController().GetUserById(User.Identity.GetUserId());
                StockProductReceipt model = await _service.FindAsync(instance.Id.ToString(), applicationUser.Id, applicationUser.OrgId.ToString());
                ...
                try
                {
                    if (TryValidateModel(model))
                    {
                        ...
                        var membersToCompare = new List<string>() { "Serial", "ProductId", "JobNo", "CaseNo", "TaskNo", "Quantity", "Weight", "WeightUnitId", "ExtraQuantity", "Expenses", "Authorized", "Active" };
        ...
        public async Task<ActionResult> DeleteConfirmed(Guid id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                {
                    StockProductReceipt model = await _service.FindAsync(id.ToString(), applicationUser.Id, applicationUser.OrgId.ToString());
                    parentGuid = model.StockReceiptId.ToString();
                    string logEntry = "Deleted StockProductReceipt: Id=" + model.Id + " StockReceiptId=" + model.StockReceiptId + " ProductId=" + model.ProductId + "JobNo=" + model.JobNo + "CaseNo=" + model.CaseNo + "TaskNo=" + model.TaskNo;
