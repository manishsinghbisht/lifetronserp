...
namespace Lifetrons.Erp.Reports.Controllers
{
    ...
    public class PrintJobIssueController : BaseController
    {
        [Dependency]
        ...
        public ActionResult Print(string reportFormatType, string id)
        {
            ...
            string path = Path.Combine(Server.MapPath("~/Areas/Reports/Rdlc"), "JobIssue.rdlc");
            ...
            var headData = JobIssueHeadService.Select(id, applicationUser.Id, applicationUser.OrgId.ToString());
            ...
            var detailsData1 = JobProductIssueService.SelectLineItems(id, applicationUser.Id, applicationUser.OrgId.ToString());
            var detailDataTable1 = CreateDetailDataTable(detailsData1);

            //Create item lineItems for report
            var detailsData2 = JobItemIssueService.SelectLineItems(id, applicationUser.Id, applicationUser.OrgId.ToString());
        ...
        private DataTable CreateHeadDataTable(IEnumerable<JobIssueHead> headData)
        {
            ...
            foreach (DataRow row in headDataTable.Rows)
            {
                ...
                //row["IssuedByProcessName"] = ProcessService.Find(row["IssuedByProcessId"].ToString(), applicationUser.Id, applicationUser.OrgId.ToString()).Name;
                //row["IssuedToProcessName"] = ProcessService.Find(row["IssuedToProcessId"].ToString(), applicationUser.Id, applicationUser.OrgId.ToString()).Name;
                //row["EmployeeName"] = EmployeeService.Find(row["EmployeeId"].ToString(), applicationUser.Id, applicationUser.OrgId.ToString()).Name;
        ...
        private DataTable CreateDetailDataTable(IEnumerable<JobProductIssue> detailData)
        {
        ...
        private DataTable CreateDetailDataTable(IEnumerable<JobItemIssue> detailData)
        {
            var detailDataTable = Lifetrons.Erp.Data.ExtensionMethods.ToDataTable(detailData);
