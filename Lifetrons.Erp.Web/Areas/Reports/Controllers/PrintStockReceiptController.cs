...
namespace Lifetrons.Erp.Reports.Controllers
{
    ...
    public class PrintStockReceiptController : BaseController
    {
        [Dependency]
        public IAspNetUserService AspNetUserService { get; set; }

        [Dependency]
        public IStockReceiptHeadService StockReceiptHeadService { get; set; }

        [Dependency]
        public IStockItemReceiptService StockItemReceiptDetailService { get; set; }

        [Dependency]
        public IStockProductReceiptService StockProductReceiptDetailService { get; set; }
        ...
        public ActionResult Print(string reportFormatType, string id)
        {
            ...
            string path = Path.Combine(Server.MapPath("~/Areas/Reports/Rdlc"), "StockReceipt.rdlc");
            ...
            var headData = StockReceiptHeadService.Select(id, applicationUser.Id, applicationUser.OrgId.ToString());
            ...
            var items = StockItemReceiptDetailService.SelectLineItems(id, applicationUser.Id, applicationUser.OrgId.ToString());
            var itemsDataTable = CreateItemsDataTable(items);

            //Create products for report
            var products = StockProductReceiptDetailService.SelectLineItems(id, applicationUser.Id, applicationUser.OrgId.ToString());
        ...
        private DataTable CreateHeadDataTable(IEnumerable<StockReceiptHead> headData)
        {
        ...
        private DataTable CreateItemsDataTable(IEnumerable<StockItemReceipt> detailData)
        {
        ...
        private DataTable CreateProductsDataTable(IEnumerable<StockProductReceipt> detailData)
        {
            var detailDataTable = Lifetrons.Erp.Data.ExtensionMethods.ToDataTable(detailData);
