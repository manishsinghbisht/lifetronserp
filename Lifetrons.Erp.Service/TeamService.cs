...
namespace Lifetrons.Erp.Service
{
    public class TeamService : Service<Team>, ITeamService
    {
        ...
        public TeamService(IRepositoryAsync<Team> repository, IAspNetUserService aspNetUserService)
            : base(repository)
        {
        ...
        public async Task<IEnumerable<Team>> GetAsync(string userId, string orgId, string departmentId = "")
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;

            System.Collections.Generic.IEnumerable<Team> task;
            ...
            {
                Guid deptIdGuid = departmentId.ToSysGuid();
                task = await _repository.Query(p => p.Active & p.OrgId == orgIdGuid & p.DepartmentId == deptIdGuid)
                 .Include(p => p.AspNetUser)
                 .Include(p => p.AspNetUser1)
                 .Include(p => p.Department)
                 .SelectAsync();                 
        ...
        public Team Find(string id, string userId, string orgId)
        {
        ...
        public async Task<Team> FindAsync(string id, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(id) || string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        public Team Create(Team param, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        public void Update(Team param, string userId, string orgId)
        {
        ...
        public IEnumerable<Team> GetPagedList(int pageNumber, int pageSize, out int totalRecords, string userId, string orgId)
        {
            Guid orgIdGuid = orgId.ToSysGuid();
