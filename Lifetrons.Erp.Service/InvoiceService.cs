...
namespace Lifetrons.Erp.Service
{
    public class InvoiceService : Service<Invoice>, IInvoiceService
    {
        ...
        public InvoiceService(IRepositoryAsync<Invoice> repository, IAspNetUserService aspNetUserService)
            : base(repository)
        {
        ...
        public IEnumerable<Invoice> Get(string invoiceId, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;

            Guid orgIdGuid = orgId.ToSysGuid();
            Guid invoiceIdGuid = invoiceId.ToSysGuid();

            var task = _repository.Query(p => p.Active & p.OrgId == orgIdGuid & p.Id == invoiceIdGuid)
        ...
        public async Task<IEnumerable<Invoice>> GetAsync(string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;

            Guid orgIdGuid = orgId.ToSysGuid();

            var task = await _repository.Query(p => p.Active & p.OrgId == orgIdGuid & p.OwnerId == userId)
              .Include(p => p.AspNetUser)
              .Include(p => p.AspNetUser1)
              .Include(p => p.AspNetUser2) //Owner
              .Include(p => p.Address) //Billing
        ...
        public async Task<Invoice> FindAsync(string id, string userId, string orgId)
        {
        ...
        public Invoice Create(Invoice param, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        public void Update(Invoice param, string userId, string orgId)
        {
        ...
        public IEnumerable<Invoice> GetPagedList(int pageNumber, int pageSize, out int totalRecords, string userId, string orgId)
        {
            Guid orgIdGuid = orgId.ToSysGuid();
