...
{
    public class StockReceiptHeadService : Service<StockReceiptHead>, IStockReceiptHeadService
    {
        [Dependency]
        public Repository<StockItemReceipt> StockItemLineItemRepository { get; set; }

        [Dependency]
        public Repository<StockProductReceipt> StockProductLineItemRepository { get; set; }
        
        private readonly IUnitOfWork _unitOfWork;
        private readonly IRepositoryAsync<StockReceiptHead> _repository;
        private readonly IAspNetUserService _aspNetUserService;

        public StockReceiptHeadService(IRepositoryAsync<StockReceiptHead> repository, IAspNetUserService aspNetUserService, IUnitOfWork unitOfWork)
            : base(repository)
        {
        ...
        public IEnumerable<StockReceiptHead> Select(string id, string userId, string orgId)
        {
        ...
        public async Task<IEnumerable<StockReceiptHead>> SelectAsync(string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        public StockReceiptHead Find(string id, string userId, string orgId)
        {
        ...
        public async Task<StockReceiptHead> FindAsync(string id, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(id) || string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        public StockReceiptHead Create(StockReceiptHead param, string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
        ...
        private void Validate(StockReceiptHead param, AspNetUser applicationUser)
        {
        ...
        public void Update(StockReceiptHead param, string userId, string orgId)
        {
        ...
        public new void Delete(StockReceiptHead model)
        {
            //// Begin transaction
            _unitOfWork.BeginTransaction();
            try
            {
                IRepository<StockItemReceipt> stockItemLineItemRepository = StockItemLineItemRepository;
                IRepository<StockProductReceipt> stockProductLineItemRepository = StockProductLineItemRepository;

                //Delete details records
                var lineItems = stockItemLineItemRepository.Query(p => p.StockReceiptId == model.Id).Select();
                lineItems.ForEach(stockItemLineItemRepository.Delete);

                var lineProducts = stockItemLineItemRepository.Query(p => p.StockReceiptId == model.Id).Select();
        ...
        public IEnumerable<StockReceiptHead> GetPagedList(int pageNumber, int pageSize, out int totalRecords, string userId, string orgId)
        {
            Guid orgIdGuid = orgId.ToSysGuid();
