...
{
    public class NoticeBoardService : Service<NoticeBoard>, INoticeBoardService
    {
        private readonly IRepositoryAsync<NoticeBoard> _repository;
        private readonly IAspNetUserService _aspNetUserService;

        public NoticeBoardService(IRepositoryAsync<NoticeBoard> repository, IAspNetUserService aspNetUserService)
            : base(repository)
        {
        ...
        public async Task<IEnumerable<NoticeBoard>> GetAsync(string userId, string orgId)
        {
        ...
        public IEnumerable<NoticeBoard> Get(string userId, string orgId)
        {
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId)) return null;
            ...
            var task = _repository.Query(p => p.Active & p.OrgId == orgIdGuid)
              .Include(p => p.AspNetUser)
              .Include(p => p.AspNetUser1)
              .Select();
        ...
        public async Task<NoticeBoard> FindAsync(string id, string userId, string orgId)
        {
        ...
        public NoticeBoard Create(NoticeBoard param, string userId, string orgId)
        {
        ...
        public void Update(NoticeBoard param, string userId, string orgId)
        {
            if (param == null || string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(orgId))
                throw new ApplicationException("Parameters cannot be null or empty");
        ...
        public IEnumerable<NoticeBoard> GetPagedList(int pageNumber, int pageSize, out int totalRecords, string userId, string orgId)
        {
            Guid orgIdGuid = orgId.ToSysGuid();
            if (orgIdGuid == Guid.Empty)
            {
